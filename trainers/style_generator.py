...
class BaseStyleGenerator(nn.Module):
    def __init__(self, cfg, classnames, clip_model, device):
        ...
        self.tokenized_base_text = self.tokenized_base_text.to("cpu")

        with torch.no_grad():
            self.base_embedding = clip_model.token_embedding(self.tokenized_base_text)
...
if __name__ == "__main__" :
    state_dict = torch.load("/mnt/disk1/theanh28/DPStyler/PromptStyler/output/pacs/vitb16_clip/PS_re_train_style/seed1/checkpoint/model.pth")
    style_embedding = state_dict["style_embedding"]
    style_embedding.requires_grad_(False)
    x = style_embedding
    print(x)
    #print(f"load weight from {weight_save_path}")
